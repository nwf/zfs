...
usage(void)
{
	...
	(void) fprintf(stderr,
	    ...
	    "    scrub [-EPRTnrv] [-D ddt_class] [-i scan_interval]\n"
	    "          [-G gap] [-H hard_factor] [-M physmem] <pool>\n"
	    "\n"
...
static void
zhack_print_vdev(const char *name, nvlist_t *nv, int depth)
{
	nvlist_t **child;
	uint_t c, children;
...
zhack_do_scrub(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "D:EG:H:PRTi:S:nrv")) != -1) {
		switch (c) {
		case 'D':
		...
		{
			char *endptr = NULL;
			uint64_t fact = strtoul(optarg, &endptr, 0);
			if ((errno == 0) && (*endptr == '\0') &&
			    (fact >= 1) && (fact <= 1000)) {
		...
		{
			char *endptr = NULL;
			uint64_t mem = strtoul(optarg, &endptr, 0);
			if ((errno == 0) && (*endptr == '\0') &&
			    (mem >= (1 << 15)) &&
			    (mem <= sysconf(_SC_PHYS_PAGES))) {
				physmem_override = mem;
			} else {
				fatal(NULL, FTAG,
		...
		case 'n':
			...
			break;
#if 0
		// This function is too horrifying to contemplate calling.
		case 'o':
	...
	zhack_spa_open(argv[optind], B_FALSE, B_FALSE, FTAG, &spa);

	if (physmem_override != 0) {
		if (verbose) {
			fprintf(stderr,
			    "Pool imported, overriding physmem...\n");
		}
		physmem = physmem_override;
	}
	...
	while (dsl_scan_scrubbing(spa_get_dsl(spa))) {
		...
		fprintf(stderr,
		    ...
		    "\n",
		    now, now - scn->scn_last_checkpoint, dp->dp_tx.tx_open_txg,
		    scnp->scn_state, scn->scn_is_sorted, scnp->scn_flags,
