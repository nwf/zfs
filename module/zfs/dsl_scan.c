 ...
 * hot as possible.
 */
uint64_t zfs_scan_dequeue_run_target_ms =	1000;
...
scan_ds_queue_sync(dsl_scan_t *scn, dmu_tx_t *tx)
{
	...
	scn->scn_phys.scn_queue_obj = zap_create(dp->dp_meta_objset, ot,
	    DMU_OT_NONE, 0, tx);
	for (sds = avl_first(&scn->scn_queue);
...
dsl_scan_sync(dsl_pool_t *dp, dmu_tx_t *tx)
{
	...
	if (scn->scn_done_txg == tx->tx_txg ||
		...
		/* finished with scan. */
		zfs_dbgmsg("txg %llu scan complete\n", tx->tx_txg);
...
dsl_resilver_restart(dsl_pool_t *dp, uint64_t txg)
{
	...
	zfs_dbgmsg("restarting resilver txg=%llu\n", txg);
}
